...
// ─── Warehouse colors (harmonious categorical palette) ──────────────────────
const WH_COLORS = CATEGORICAL
const whColor = (idx) => WH_COLORS[idx % WH_COLORS.length]

// ════════════════════════════════════════════════════════════════════════════
//  TAB 1 — PLT Lateral Transshipment
// ════════════════════════════════════════════════════════════════════════════
const PLTTab = ({ runId, products, warehouses }) => {
  ...
  // Chart by period — grouped by SOURCE warehouse (from_wh) to keep ≤ 6 colors (avoid a 22-pair rainbow).
  // Each period column = total quantity transferred OUT from each source warehouse in that period.
  const periods = [...new Set(transfers.map((t) => t.time_period))].sort((a, b) => a - b)
  const barData = periods.map((t) => {
    const row = { period: `Period ${t}` }
  ...
  if (error) return <Alert type="error" message="Error loading PLT data" description={String(error)} />
    ...
    <div className="space-y-4">
      ...
      <Card size="small">
        <Row gutter={12} align="middle">
          <Col flex="none"><FilterOutlined /> <strong>Filter:</strong></Col>
          <Col flex="200px">
            <Select allowClear value={pltProduct} onChange={setPltProduct}
              style={{ width: '100%' }} placeholder="Product" showSearch optionFilterProp="children">
              {products.map((p) => <Option key={p} value={p}>{p}</Option>)}
            </Select>
          </Col>
          <Col flex="130px">
            <Select allowClear value={pltFromWh} onChange={setPltFromWh}
              style={{ width: '100%' }} placeholder="From warehouse">
              {warehouses.map((w) => <Option key={w} value={w}>{w}</Option>)}
            </Select>
          </Col>
          <Col flex="130px">
            <Select allowClear value={pltToWh} onChange={setPltToWh}
              style={{ width: '100%' }} placeholder="To warehouse">
              {warehouses.map((w) => <Option key={w} value={w}>{w}</Option>)}
            </Select>
          </Col>
        </Row>
      ...
      {!has_plt || transfers.length === 0 ? (
        <Empty description="No PLT transfers in this run" style={{ marginTop: 48 }} />
      ) : (
        <>
          {/* Overview cards */}
          <Row gutter={16}>
            <Col span={6}>
              <Card size="small">
                <Statistic title="Total transfer quantity" prefix={<SwapOutlined />}
                  value={transfers.reduce((s, r) => s + r.qty, 0).toLocaleString()} suffix="units" />
              </Card>
            </Col>
            <Col span={6}>
              <Card size="small">
                <Statistic title="Warehouse pairs" prefix={<NodeIndexOutlined />}
                  value={summary.length} suffix="pairs" />
              </Card>
            </Col>
            <Col span={6}>
              <Card size="small">
                <Statistic title="Number of transfers" value={transfers.length} suffix="transfers" />
              </Card>
            </Col>
            <Col span={6}>
              <Card size="small">
                <Statistic title="Periods with PLT"
                  value={[...new Set(transfers.map((t) => t.time_period))].length} suffix="periods" />
              </Card>
            </Col>
          </Row>

          {/* Matrix */}
          <Card title="PLT matrix — total transfer quantity by warehouse pair" size="small">
            <Text type="secondary" style={{ fontSize: 12 }} className="block mb-2">
              Unit: product. Values include decimals because they are cumulative totals across multiple transfers — some transfers include residual units (not rounded to a case-pack)
            </Text>
          ...
          {/* Chart by period */}
          {barData.length > 0 && (
            <Card title="PLT quantity by period (by source warehouse)" size="small">
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={barData} margin={{ top: 8, right: 24, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} />
                  <XAxis dataKey="period" />
                  <YAxis />
                  <Tooltip formatter={(v, name) => [Number(v).toLocaleString('en-US'), `From ${name}`]} />
                  <Legend formatter={(v) => `From ${v}`} />
                  {barKeys.map((k, i) => (
          ...
          {/* Detail table */}
          <Card title="PLT transfer detail" size="small">
            <Table dataSource={transfers} rowKey="plt_id" size="small"
              pagination={{ pageSize: 20, showSizeChanger: true }}
              columns={[
                { title: 'Product', dataIndex: 'product_id', key: 'pid', width: 130 },
                { title: 'From warehouse', dataIndex: 'from_warehouse_id', key: 'from',
                  render: (v) => <span className="font-medium" style={{ color: NEUTRAL[700] }}>{v}</span> },
                { title: 'To warehouse', dataIndex: 'to_warehouse_id', key: 'to',
                  render: (v) => <span className="font-medium" style={{ color: NEUTRAL[700] }}>{v}</span> },
                { title: 'Period', dataIndex: 'time_period', key: 't',
...
// ════════════════════════════════════════════════════════════════════════════
//  TAB 2 — Inventory Trajectory By Warehouse
// ════════════════════════════════════════════════════════════════════════════
const METRIC_OPTIONS = [
  { value: 'total_net_inventory', label: 'Inventory (net)' },
...
const InventoryTrajectoryTab = ({ runId }) => {
  ...
  if (loading) return <div className="flex justify-center py-16"><Spin size="large" /></div>
  if (error) return <Alert type="error" message="Error loading inventory data" description={String(error)} />
  if (!warehouses.length) return <Empty description="No inventory data available" style={{ marginTop: 48 }} />
    ...
    <div className="space-y-4">
      ...
      <Card size="small">
        <Row align="middle" gutter={12}>
          <Col flex="none"><strong>Metric:</strong></Col>
          <Col flex="220px">
      ...
      {/* Line chart */}
      <Card title={`${metricLabel} trajectory by warehouse over periods`} size="small">
      ...
      {/* Summary table */}
      <Card title="Warehouse Summary" size="small">
        <Table dataSource={warehouses} rowKey="warehouse_id" size="small" pagination={false}
          columns={[
            ...
            { title: 'Total backorder', dataIndex: 'total_backorder', align: 'right',
            ...
            { title: 'Total overstock', dataIndex: 'total_overstock', align: 'right',
              render: (v) => <span style={{ color: v > 0 ? SEMANTIC.warn : undefined }}>{Number(v).toLocaleString()}</span> },
            { title: 'Total shortage', dataIndex: 'total_shortage', align: 'right',
              render: (v) => <span style={{ color: v > 0 ? SEMANTIC.warn : undefined }}>{Number(v).toLocaleString()}</span> },
          ]}
        />
      </Card>
...
const AllocationTab = ({ runId, products, warehouses }) => {
  ...
  // Flatten inventory dynamics into chartData
  const chartData = []
  ...
  // Inventory chart by period (grouped by warehouse)
  ...
  return (
    <div className="space-y-4">
      {/* Filters — product selection required */}
      <Card size="small">
        <Row gutter={12} align="middle">
          <Col flex="none"><FilterOutlined /> <strong>Filter:</strong></Col>
          <Col flex="200px">
            <Select value={filterProduct} onChange={setFilterProduct} allowClear
              style={{ width: '100%' }} placeholder="Select a product (required)"
          ...
          <Col flex="140px">
            <Select value={filterWarehouse} onChange={setFilterWarehouse} allowClear
              style={{ width: '100%' }} placeholder="Warehouse (optional)">
              {warehouses.map((w) => <Option key={w} value={w}>{w}</Option>)}
            </Select>
          </Col>
        </Row>
      ...
      {!filterProduct ? (
        <Alert type="info" message="Please select a product to view allocation detail" />
      ) : loading ? (
        <div className="flex justify-center py-16"><Spin size="large" /></div>
      ) : error ? (
        <Alert type="error" message="Error loading data" description={String(error)} />
      ) : (
        <>
          {/* Overview cards */}
          <Row gutter={16}>
            ...
            <Col span={6}>
              <Card size="small">
                <Statistic title="Packing violations"
                  valueStyle={{ color: allocations.filter((a) => a.penalty_flag).length > 0 ? SEMANTIC.warn : undefined }}
                  value={allocations.filter((a) => a.penalty_flag).length} suffix="rows" />
              </Card>
            </Col>
          </Row>

          {/* Inventory chart by warehouse × period */}
          {lineData.length > 0 && (
            <Card title={`Inventory trajectory — ${filterProduct}`} size="small">
          ...
          {/* Detail table */}
          <Card title={`Allocation Detail — ${filterProduct}`} size="small">
            <Table dataSource={allocations} rowKey={(r) => `${r.product_id}-${r.warehouse_id}-${r.time_period}`}
              ...
              columns={[
                ...
                { title: 'Period', dataIndex: 'time_period', key: 't',
                  sorter: (a, b) => a.time_period - b.time_period },
                { title: 'Case-pack (Q)', dataIndex: 'q_case_pack', key: 'q', align: 'right',
                  render: (v) => Number(v).toLocaleString(),
                  sorter: (a, b) => a.q_case_pack - b.q_case_pack },
                { title: 'Residual units (r)', dataIndex: 'r_residual_units', key: 'r', align: 'right',
                  render: (v) => Number(v).toLocaleString() },
                { title: 'Inventory (I)', dataIndex: 'net_inventory', key: 'inv', align: 'right',
                  render: (v) => (
                ...
                { title: 'Violation', dataIndex: 'penalty_flag', key: 'pen',
...
const AllocationInventoryDashboard = () => {
  ...
  const tabItems = [
    {
      key: 'plt',
      label: <span><SwapOutlined /> Lateral Transshipment (PLT)</span>,
      children: <PLTTab runId={activeRunId} products={products} warehouses={warehouses} />,
    },
    {
      key: 'trajectory',
      label: <span><LineChartOutlined /> Inventory Trajectory By Warehouse</span>,
      children: <InventoryTrajectoryTab runId={activeRunId} />,
    },
    {
      key: 'allocation',
      label: <span><AppstoreOutlined /> Allocation By Product</span>,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="B3. Allocation & Inventory Dynamics"
        subtitle="Lateral transshipment between warehouses · Inventory trajectory by warehouse · Allocation detail by product"
      />

      {/* Run selector */}
      <Card size="small">
        <Row gutter={12} align="middle">
          <Col flex="none"><strong>Optimization Run:</strong></Col>
          <Col flex="360px">
            <Select loading={runsLoading} value={activeRunId} onChange={setActiveRunId}
              style={{ width: '100%' }} placeholder="Select an optimization run"
              options={runs.map((r) => ({
                value: r.run_id,
      ...
      {!activeRunId ? (
        <Alert type="info" message="Select an optimization run to view results" />
      ) : (
